...
function Add_vatthe($pid)
{
	...
	$loai_ds=select_hientrang("",_loaidisan);
	...
	if($pid){
		...
		$loai_ds=select_hientrang($row['loai_ds'],_loaidisan);
	...
	Header("Location: ".$portalurl."/admin.php?op=Ds_vatthe");
	...
	Header("Location: ".$portalurl."/admin.php?op=Add_vatthe_imgs&pid=".$pids."");
	}
	}
    if($pid){
...
function Sub_xahuyen($country){
	...
	echo '<select name="xa" class="form-control" id="xaphuong">
			...
			';
			$result = $db->sql_query("Select id,name from " . $prefix . "_xahuyen where id_xa='".intval($country)."'    ORDER BY name asc");
...
include 'admin/include/Topmenu_header.php';
	...
	$xtpl = new XTemplate('admin/htmls/Vatthe/Add_vatthe_imgs.html');
    $xtpl->assign("portalurl", $portalurl);
	 $xtpl->assign("pid", $pid);
...
//                print_r($_FILES);
                ...
                foreach ($_FILES['files']['name'] as $f => $name) {
                    ...
                    move_uploaded_file($_FILES["files"]["tmp_name"][$f], $path.$name_upload) ;
					 ...
					 if($_POST['ok_num']) {
						 Header("Location: ".$portalurl."/admin.php?op=Add_vatthe_imgs&pid=".$pid."");
					 }else if($_POST['buoc_tiep']){
						 Header("Location: ".$portalurl."/admin.php?op=Add_vatthe_video&pid=".$pid."");
					 }
...
include 'admin/include/Topmenu_header.php';
	...
	$xtpl = new XTemplate('admin/htmls/Vatthe/Add_vatthe_video.html');
    $xtpl->assign("portalurl", $portalurl);
	 $xtpl->assign("pid", $pid);
	if($_POST['ok_num']) 
{ 
