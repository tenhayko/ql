...
function Add_vatthe_hungha($pid)
{
   global $prefix, $currentlang, $aid, $sortorder, $portalurl, $sitename, $db, $nam_thang, $cv_id_hs;
	...
	$bien_bang = $prefix.'_vatthe_dinhkem';

	$xtpl->assign("title", $title);
	$xtpl->assign("bien_bang", $bien_bang);
	$xtpl->assign("select_thotu", $select_thotu);
	
	$xtpl->assign("name_ditich", $name_ditich);
	...
	$data['tenditich_2'] = mb_strtolower($_POST['tenditich'],'utf-8');
	...
	$data['noidung']=$_POST['noidung'];
		...
		//                print_r($_FILES);
						...
						foreach ($_FILES['dinhkem']['name'] as $f_dk => $name_dk) {
							...
							$time = time();
							$name_fn = $time."_".Convert_vi_to_en($name_dk);
							//echo $name_fn;
							//exit;
							move_uploaded_file($_FILES["dinhkem"]["tmp_name"][$f_dk], $path_dk.$name_fn);
		
							 $result = $db->sql_query("insert into ".$prefix."_vatthe_dinhkem(id,id_vatthe,dk_file) values(null,'$pid','$name_fn')"); 
		...
		//                print_r($_FILES);
						...
						foreach ($_FILES['dinhkem']['name'] as $f_dk => $name_dk) {
							...
							move_uploaded_file($_FILES["dinhkem"]["tmp_name"][$f_dk], $path_dk.$name_fn) ;
							 ...
							 $result = $db->sql_query("insert into ".$prefix."_vatthe_dinhkem(id,id_vatthe,dk_file) values(null,'$pids','$name_fn')"); 
	...
	while($row_vatthe = $db->sql_fetchrow($sgl_Vatthe)) 
		{
			...
			$data['tenditich_2'] = mb_strtolower($row_vatthe['tenditich'],'utf-8');
			
			updateQuery('Update',$prefix.'_vatthe', $data,"id = '".$row_vatthe['id']."'"); 
...
function Add_vatthe_imgs_hungha($pid)
{
    ...
    $xtpl->assign("portalurl", $portalurl);
	 $xtpl->assign("pid", $pid);
	 if($pid){
	 	$name_ditich = get_name_ditich($pid);
	 }
	 else{
	 	$name_ditich = "";
	 }
...
function Delete_dinhkem_cv($id,$table_xoa){
	   global $prefix, $currentlang, $aid, $sortorder, $portalurl, $sitename, $db, $nam_thang, $cv_id_hs;


	$result_filedk = $db->sql_query("SELECT dk_file FROM ".$prefix."_vatthe_dinhkem  where id='" . intval($id) . "' ");
						 
				list($file_dk) = $db->sql_fetchrow($result_filedk);
				
				if (file_exists('images/Image/'.$file_dk))
					{
						//chown('images/Image/'.$file_dk,666);
						unlink('images/Image/'.$file_dk);
					}
					
   $db->sql_query("DELETE FROM ".$prefix."_vatthe_dinhkem WHERE id='".intval($id)."'");
    return $db;
}
//---------------------------


function Xephang_ditich()
{
    global $prefix, $currentlang, $aid, $sortorder, $portalurl, $sitename, $db, $nam_thang, $cv_id_hs;
    include 'admin/include/Topmenu_header.php';
    include 'header.php';
    ...
    $xtpl = new XTemplate('admin/htmls/Hungha/Xephang_ditich.html');
    $xtpl->assign("portalurl", $portalurl);
	
	
	
	$result= $db->sql_query("SELECT id,name from " . $prefix . "_xephang
									  order by id asc ");
        ...
        while($row = $db->sql_fetchrow($result)){
			...
			$result_sum = $db->sql_query("SELECT count(id) FROM ".$prefix."_vatthe  where xephang='" . intval($row['id']) . "' ");
						 
				list($tongso) = $db->sql_fetchrow($result_sum);
				
			
			
			
			$xtpl->assign("tongso", $tongso);
			$xtpl->assign("row", $row);
			$xtpl->parse("MAIN.xephang");
		}
...
switch ($op) {
		...
		Delete_dinhkem_cv($id,$table_xoa);
		break;
		case "Xephang_ditich":
		Xephang_ditich();
		break;
